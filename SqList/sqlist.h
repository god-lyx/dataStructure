...
public:
    ...
    int inputList(int = 8);
    SqList<ElemType> operator+(SqList<ElemType>& L);//算符重载，并运算
    //SqList<ElemType> operator^(const SqList<ElemType>& L);//算符重载，交运算
    //SqList<ElemType> operator-(const SqList<ElemType>& L);//算符重载，差运算
    void dispList();//打印顺序表
...
template<class ElemType>
SqList<ElemType> SqList<ElemType>::operator+(SqList<ElemType>& L2){//并运算
    SqList<ElemType> L3;
    int i = 0;
    for (i = 0; i < length; i++) {
